...
public class TicTacToe {
    ...
    private static boolean isWin(char symbol) {
        ...
        for (int i = 0; i < SIZE; i++) {
            ...
            int countColumn = 0;
            for (int j = 0; j < SIZE; j++) {
                if (map[i][j] == symbol) countRow = countRow + 1;
                if (map[j][i] == symbol) countColumn = countColumn + 1;
            }
        ...
        int countDiagonal = 0;
        for (int i = 0; i < SIZE; i++) {
            if (map[i][i] == symbol) countDiagonal = countDiagonal + 1;
        }
        if (countDiagonal == 3) return true;
        else countDiagonal = 0;

        for (int i = 0; i < SIZE; i++) {
            if (map[i][SIZE - 1 - i] == symbol) countDiagonal = countDiagonal + 1;
        }
        return countDiagonal == 3;
